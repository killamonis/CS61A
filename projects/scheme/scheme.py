...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        for i in range(len(python_formals)):
...
class LambdaProcedure(Procedure):
    ...
    def make_call_frame(self, args, env):
        ...
        # BEGIN PROBLEM 11
        env.make_child_frame(self.formals, args)
        # END PROBLEM 11
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target): 
        ...
        lambda_body = LambdaProcedure(formals, body, env)
        env.define(function_name,  lambda_body)
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        vals = scheme_eval(bindings.rest.first, env)
        values = Pair(vals, values)
