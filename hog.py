...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        if who == 0:
            previous_score0 = turn_score0
            num_rolls0 = strategy0(score0, score1)
            turn_score0 = take_turn(num_rolls0, score1, dice)
            score0 += turn_score0  
        else:
            previous_score1 = turn_score1
            num_rolls1 = strategy1(score1, score0)
            turn_score1 = take_turn(num_rolls1, score0, dice)
            score1 += turn_score1
        if feral_hogs:
            if who == 0 and abs(previous_score0 - num_rolls0) == 2:
                score0 += 3
            if who == 1 and abs(previous_score1 - num_rolls1) == 2:
                score1 += 3
...
def announce_highest(who, prev_high=0, prev_score=0):
    ...
    def say(score0, score1):
        new_high = prev_high
        if who == 0:
            this_turn = score0 - prev_score
            new_score = score0
        else:
            this_turn = score1 - prev_score
            new_score = score1
        if this_turn > prev_high:
            new_high = this_turn
            print(this_turn, "point(s)! That's the biggest gain yet for Player", who)
